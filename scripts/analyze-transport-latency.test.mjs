...
function line({ seq = 1, status = 202, open = 0, headers = 4400, total = 4402, reuse = 1 } = {}) {
  return `[Transport] seq=${seq} status=${status} socket_open_before=${open} headers_ms=${headers} total_ms=${total} reusable=${reuse}`;
}
...
test("ignores unrelated lines and discards serial prefixes", () => {
  ...
  assert.deepEqual(parseRecord(`12:34:56 > ${line()}\r`), {
    seq: 1, status: 202, socketOpenBefore: 0, headersMs: 4400,
    totalMs: 4402, reusable: 1, accepted: true,
  });
});
...
test("rejects malformed and incomplete records without leaking their contents", async () => {
  ...
  for (const value of ["[Transport]", `${line()} extra=1`, `${line()} ${secret}`]) {
...
test("rejects invalid numeric ranges and inconsistent response state", () => {
  for (const value of [
    line({ seq: 4294967296 }), line({ headers: -1 }), line({ total: 4294967296 }),
    line({ headers: 9, total: 8 }), line({ status: 0, reuse: 0 }),
    line({ status: 600, reuse: 0 }), line({ status: -32769, reuse: 0 }),
    line({ status: 429, reuse: 1 }), line({ open: 2 }), line({ reuse: 2 }),
  ]) assert.throws(() => parseRecord(value));
});

test("accepts unsigned boundaries, zero durations and negative transport errors", () => {
  assert.equal(parseRecord(line({ seq: 4294967295, headers: 0, total: 0 })).seq, 4294967295);
  assert.equal(parseRecord(line({ headers: 4294967295, total: 4294967295 })).totalMs, 4294967295);
  assert.equal(parseRecord(line({ status: -11, reuse: 0 })).accepted, false);
});

test("separates accepted requests, failures and open-socket candidates", async () => {
  const result = await analyzeLines([
    "[GNSS] connected", line(), line({ seq: 2, open: 1, headers: 20, total: 23 }),
...
test("uses exact nearest-rank percentiles without blending cold and warm candidates", async () => {
  const lines = Array.from({ length: 100 }, (_, index) => line({
    seq: index + 1, open: 1, headers: index + 1, total: index + 3,
  })).reverse();
  lines.push(line({ seq: 101, headers: 4400, total: 4402 }));
  const result = await analyzeLines(lines);
  assert.deepEqual(result.groups.accepted_socket_open_candidate.headersMs, {
    min: 1, p50: 50, p95: 95, p99: 99, max: 100,
  });
  assert.equal(result.groups.accepted_socket_closed.headersMs.p50, 4400);
});

test("empty groups are explicit and duplicate sequence attempts are not removed", async () => {
  const result = await analyzeLines([line(), line({ open: 1 })]);
  assert.equal(result.samples, 2);
  assert.deepEqual(result.groups.nonaccepted_socket_closed, {
    count: 0, retainedAfterResponse: 0, headersMs: null, cleanupMs: null, totalMs: null,
  });
});

test("accepted status remains accepted even when cleanup cannot retain the socket", async () => {
...
test("fails on absent data instead of emitting a misleading empty benchmark", async () => {
...
test("limits both completed and partial long lines", async () => {
  ...
  await assert.rejects(collect(readBoundedLines([`${value}x\n`])), /line length limit/);
  await assert.rejects(collect(readBoundedLines([value, "x"])), /line length limit/);
});
...
test("enforces a byte budget rather than a character count", async () => {
  ...
  for (const limit of [0, -1, 0.5, Number.NaN, Number.POSITIVE_INFINITY]) {
    await assert.rejects(collect(readBoundedLines(["x"], limit)), /invalid byte limit/);
  }
});

test("propagates input stream failures and closes streams on parser failure", async () => {
...
test("reads UTF-8 files without including unrelated sensitive lines in the report", async (t) => {
...
test("CLI emits ordered independent captures and no TLS-only claim", async (t) => {
  const directory = await temporaryFiles(t);
  const before = join(directory, "before.log");
  const after = join(directory, "after.log");
  await writeFile(before, line());
  await writeFile(after, line({ open: 1, headers: 20, total: 21 }));
...
test("CLI errors do not publish a partial report or filesystem paths", async (t) => {
  const directory = await temporaryFiles(t);
